...
# ===============================
# 4. Gửi tín hiệu sang ESP32
# - Khi gửi: chỉ remove_accents (không đổi hoa/thường)
# - LED: kiểm tra biến không dấu ở dạng lower để so sánh "khong"
# ===============================
try:
    esp32 = serial.Serial('COM4', 115200, timeout=1)
    time.sleep(2)

    # Biển số gửi: đã xử lý rồi, chỉ bỏ dấu (không lower)
    plate_to_send = remove_accents(bien_so_hienthi)
    esp32.write(f"PLATE:{plate_to_send}\n".encode())

    # Tỉnh gửi: chỉ bỏ dấu (không lower)
    province_to_send = remove_accents(tinh)
    esp32.write(f"PROVINCE:{province_to_send}\n".encode())

    # LED: dùng biến không dấu + lower để kiểm tra chuỗi "khong"
    plate_check = remove_accents(bien_so_hienthi).lower()
    if "khong" in plate_check or "khong" in remove_accents(tinh).lower():
